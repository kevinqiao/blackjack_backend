...
export class LaunchProcessor {
    ...
    process = (game: GameModel) => {
        ...
        this.eventService.sendEvent({ name: "startPlay", topic: "", data: { round: 1 }, delay: 0 })
        let time = 0;
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (seat) {
                ...
                if (card?.no) {
                    ...
                    this.eventService.sendEvent({ name: "releaseCard", topic: "model", data: { seat: seat.no, no: card1 }, delay: time })
                    card = this.gameEngine.releaseCard(game, seat.no, seat.currentSlot);
                    if (card?.no) {
                        time = time + 300;
                        let card2 = card.no
                        seat.slots[0].cards.push(card2)
                        this.eventService.sendEvent({ name: "releaseCard", topic: "model", data: { seat: seat.no, no: card2 }, delay: time })
                    }
        ...
        if (dealerSeat) {
            ...
            if (dealerCard) {
                ...
                this.eventService.sendEvent({ name: "releaseCard", topic: "model", data: { seat: 3, no: dealerCard.no }, delay: time })
        ...
        if (typeof startSeat !== 'undefined') {
            if (startSeat.status === 0) {
                time = time + 100;
                const actionTurn: ActionTurn = { id: Date.now() + 2,gameId:game.gameId, round: 1, tableId:game.tableId, expireTime: Date.now() + Constants.TURN_INTERVAL +time, seat: startSeat.no, data: null }
                game.currentTurn = actionTurn;
                game.round=1;
                this.turnService.newActionTurn(actionTurn,time);
                return;
            }
            if (this.gameEngine.turnSeat(game, startSeat))
                return;
